...
function dstNotOwnFromDefinitionStrictlyPrimitive()
{
  ...
  let routine = function dstNotOwnFromDefinitionStrictlyPrimitive( dstContainer, srcContainer, key )
  {
    ...
    {
      _.assert( !_.consequenceIs( srcElement ) && ( _.primitiveIs( srcElement ) || _.routineIs( srcElement ) ), () => _.entity.exportStringShort( dstContainer ) + ' has non-primitive element ' + _.strQuote( key ) + ' use _.define.own instead' );
      dstContainer[ key ] = srcElement;
    }
